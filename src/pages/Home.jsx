...
const Home = () => {
  ...
  return (
    <>
      {/* Image Carousel */}
      <section className="relative bg-gray-200 m-4 md:m-8  mx-auto h-[400px] md:h-[780px] rounded-lg overflow-hidden shadow">

        {carouselImages.map((img, idx) => (
          <img
            key={idx}
            src={img || 'https://via.placeholder.com/1200x550?text=Image+Not+Found'}
            alt={`Slide ${idx + 1}`}
            className={`absolute top-0 left-0 w-full h-full object-cover transition-opacity duration-1000 ${
              idx === currentIndex ? 'opacity-100' : 'opacity-0'
            }`}
            onError={(e) => {
              e.target.src = 'https://via.placeholder.com/1200x550?text=Image+Not+Found';
            }}
          />
        ))}
